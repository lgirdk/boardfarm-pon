"""Lint and test boardfarm-pon on multiple python environments."""
...
def lint(session: nox.Session) -> None:
    ...
    session.run("flake8", ".")
    session.run("mypy", "boardfarm3_pon")


@nox.session(python=_PYTHON_VERSIONS)
def pylint(session: nox.Session) -> None:
    """Lint boardfarm-pon using pylint without dev dependencies.

    # noqa: DAR101: - session
    """
    session.install("--upgrade", "--pre", "boardfarm3")
    session.install("--upgrade", ".", "pylint==3.2.6")
    session.run("pylint", "boardfarm3_pon")
